...
contract PegSentinelReactive is AbstractReactive {
    ...
    address constant MAKERDAO_VAT = 0x35D1b3F3D7966A1DFe207aa4514C12a259A0492B;
    ...
    bytes32 constant FOLD_TOPIC = keccak256("fold(bytes32,address,int256)");

    // --- State ---
    mapping(address token => uint8 confidence) public confidence;
    mapping(address token => uint8 lastReportedConfidence)
        public lastReportedConfidence;
    ...
    constructor(
        ...
        tokenMapping[MAKERDAO_VAT] = unichainDAI;
    ...
    function _handleMakerDaoFold(
        ...
        if (data.length < 32) return;
        uint256 rateRaw = abi.decode(data, (uint256));
